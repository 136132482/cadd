...
public:
    ...
    //host=localhost dbname=pg_db user=root password=root options='-c timezone=Asia/Shanghai’
    static void init(const std::string& connStr="host=localhost dbname=pg_db user=root password=root",
                     size_t poolSize = 5) {
        // 直接使用全局Logger实例
...
//                sql_.open(soci::postgresql,
//                          "host=localhost dbname=pg_db user=root password=root");
//            }
    ...
    template<typename T>
    static std::optional<std::int64_t> insert(const T& obj) {
        Session sess;
        ...
        try {
            ...
            // 处理每个字段
            soci::table_info<T>::for_each_field_with_primary(
                    [&](auto member_ptr, const std::string& col_name,bool is_primary) {
                        if (!soci_config::is_standard_field(col_name,is_primary)) {
...
//            });
            ...
            sql << "INSERT INTO " << table << " ("
                << boost::algorithm::join(insert_columns, ", ") << ") VALUES ("
            ...
            auto inserted = queryOne(obj);
            if (!inserted) {
                Logger::Global().Error("Inserted record not found after commit");
                return std::nullopt;
            }

            auto id_member_ptr = soci::table_info<T>::template id_member<T>();
            auto id_value = (*inserted).*id_member_ptr;

            // 处理optional和non-optional类型
            if constexpr (std::is_same_v<decltype(id_value), std::optional<std::int64_t>>) {
                return id_value.value_or(-1);
            } else if constexpr (std::is_same_v<decltype(id_value), std::optional<int>>) {
                return id_value ? static_cast<std::int64_t>(*id_value) : -1;
            } else if constexpr (std::is_same_v<decltype(id_value), int>) {
                return static_cast<std::int64_t>(id_value);
            } else {
                return id_value;
            }
    ...
    template<typename T>
    static std::optional<std::vector<std::int64_t>> bulkInsert(std::vector<T>& objects) {
        if (objects.empty()) {
        ...
        Session sess;
        soci::transaction tr(sess.get());
        std::vector<std::int64_t> inserted_ids;
        std::vector<T> copiedObjects; // 存储拷贝后的对象
        // 3. 拷贝对象到新容器
        copiedObjects.reserve(objects.size());
        for (const auto& obj : objects) {
            copiedObjects.push_back(obj); // 这里发生拷贝
        }
        ...
        try {
            ...
            std::cout << "\n===== 批量插入表结构 =====" << std::endl;
            std::cout << "表名: " << table << std::endl;
            std::cout << "列名: " << boost::algorithm::join(insert_columns, ", ") << "\n" << std::endl;
            ...
            // 4. 使用queryList批量查询插入的记录（优化点）
            if (!copiedObjects.empty()) {
                // 构建查询条件：查找与原始对象匹配的记录
                auto inserted_records = queryList(copiedObjects);

                // 提取ID
                for (const auto& record : inserted_records) {
                    auto id_value = record.*id_member_ptr;
                    if constexpr (std::is_same_v<decltype(id_value), std::optional<std::int64_t>>) {
                        if (id_value) inserted_ids.push_back(*id_value);
                    } else if constexpr (std::is_same_v<decltype(id_value), std::optional<int>>) {
                        if (id_value) inserted_ids.push_back(static_cast<std::int64_t>(*id_value));
                    } else if constexpr (std::is_same_v<decltype(id_value), int>) {
                        inserted_ids.push_back(static_cast<std::int64_t>(id_value));
                    } else {
                        inserted_ids.push_back(id_value);
                    }
                }

                // 打印ID统计信息
                std::cout << "\n===== ID统计 =====" << std::endl;
                std::cout << "获取ID数量: " << inserted_ids.size() << std::endl;
                if (!inserted_ids.empty()) {
                    std::cout << "最小ID: " << *std::min_element(inserted_ids.begin(), inserted_ids.end())
                              << ", 最大ID: " << *std::max_element(inserted_ids.begin(), inserted_ids.end())
                              << ", 区间跨度: " << inserted_ids.back() - inserted_ids.front() + 1 << std::endl;
                }
            }

            // 验证插入数量
            if (inserted_ids.size() != objects.size()) {
                Logger::Global().Warning("Inserted count mismatch: expected " +
                                         std::to_string(objects.size()) +
                                         ", got " + std::to_string(inserted_ids.size()));
            }
    ...
    static bool update(const T& obj) {
        ...
        try {
            ...
            soci::table_info<T>::for_each_field_with_primary(
                    [&](auto member_ptr, const std::string& col_name, bool is_primary) {
                        if (!soci_config::is_standard_field(col_name,is_primary) && col_name != id_column) {
                            using FieldType = std::decay_t<decltype(obj.*member_ptr)>;
                            std::string val = soci_helpers::TypeConverter<FieldType>::toSQL(obj.*member_ptr);
                            // 关键改进：跳过NULL值
                            if (val != "NULL") {
    ...
    static bool remove(std::int64_t id) {
        ...
        try {
            ...
            // 类型转换处理
            if constexpr (std::is_same_v<decltype(std::declval<T>().*id_member), std::optional<std::int64_t>>) {
...
/* 批量软删除（完全修正版）*/
    template<typename T>
    static bool bulkRemove(const std::vector<std::int64_t>& ids) {
        auto start_time = std::chrono::system_clock::now();
...
/* 恢复数据（带完整日志）*/
    template<typename T>
    static bool restore(std::int64_t id) {
        auto start_time = std::chrono::system_clock::now();
    ...
    /* 批量恢复数据（带完整日志）*/
    template<typename T>
    static bool bulkRestore(const std::vector<std::int64_t>& ids) {
        auto start_time = std::chrono::system_clock::now();
...
/* 分页查询方法（复用query实现）*/
    ...
    static int update(const std::string& sql, Args&&... args) {
        Session sess;
        try {
            // 1. 参数安全转换（复用query的逻辑）
            ...
            // 2. 构建最终SQL（与query相同）
            std::vector<std::string> param_strs = { convert_param(std::forward<Args>(args))... };
            ...
            soci::statement st = (sess.get().prepare << final_sql);
            st.execute(true);
            ...
            std::cout << "[STAT] 影响行数: " << affected_rows
...
//                    }
    ...
    */
    ...
    static SafeRowset queryRaw(const std::string& sql) {
        auto session = std::make_shared<Session>(); // 保持Session生命周期
        try {
            soci::rowset<soci::row> rs = (session->get().prepare << sql);
            return {session, rs};
        } catch (const std::exception& e) {
            Logger::Global().Error("SQL查询失败: " + std::string(e.what()));
            Logger::Global().Debug("失败SQL: " + sql);
            throw;
        }
    }
    ...
    template<typename T>
    static std::optional<T> queryOne(const T& obj) {
        try {
            const auto& table = soci::table_info<T>::name;
            ...
            soci::table_info<T>::for_each_field(
                    [&](auto member_ptr, const std::string& col_name) {
                        ...
                        if (!soci_config::is_standard_field(col_name)) {
                            std::string val = soci_helpers::TypeConverter<MemberType>::toSQL(obj.*member_ptr);
                            if (val != "NULL") {
                                conditions.push_back(col_name + " = " + val);
                            }
                        }
                    }
            );
            ...
            if (conditions.empty()) {
                throw std::runtime_error("No valid conditions provided for query");
            }

            // 使用ostringstream构建SQL语句
            std::ostringstream sql;
            sql << "SELECT * FROM " << table
                << " WHERE " << boost::algorithm::join(conditions, " AND ")
                << " LIMIT 1";

            auto results = query<T>(sql.str());
            if (!results.empty()) {
                return results.front();
            }
            return std::nullopt;
        } catch (const std::exception& e) {
            std::cerr << "Query one error: " << e.what() << std::endl;
            return std::nullopt;
        }
    }
    ...
    template<typename T>
    static std::vector<T> queryList(const std::vector<T>& objs) {
        Session sess;
        std::vector<T> results;
        if (objs.empty()) return results;
...
//                                std::cout << "Member type: " << typeid(MemberType).name() << "\n";
//                                std::cout << "Value type:  " << typeid(obj.*member_ptr).name() << "\n";
//                                std::cout << "Is PostgisPoint: "
//                                          << std::is_same_v<MemberType, PostgisPoint> << "\n";
                                std::string val = soci_helpers::TypeConverter<MemberType>::toSQL(obj.*member_ptr, true);
                                if (val != "NULL") {
                                    condition_parts.push_back(col_name + " = " + val);
                                }
                            }
                        }
                );

                if (!condition_parts.empty()) {
                    where_clauses.push_back("(" + boost::algorithm::join(condition_parts, " AND ") + ")");
                }
            }

            // 2. 构建并打印SQL
            std::ostringstream sql;
            sql << "SELECT * FROM " << soci::table_info<T>::name
                << " WHERE " << boost::algorithm::join(where_clauses, " OR ")
                << " LIMIT 1000";

            std::cout << "Executing SQL:\n" << sql.str() << std::endl; // 唯一保留的输出
...
// 新增：根据ID查询单个对象
    template<typename T>
    static std::optional<T> queryById(std::int64_t id) {
        try {
            const auto& table = soci::table_info<T>::name;
            const auto& id_column = soci::table_info<T>::id_column();
            ...
            std::cerr << "Query by ID error: " << e.what() << std::endl;
            return std::nullopt;
        }
    }
    ...
    static bool create_next_month_partition(
            const std::string& table_name,
            const std::string& partition_key = "created_at",
            const std::string& comment_prefix = "日志分区"
    ) {
        try {
            Session sess;
            soci::transaction tr(sess.get());
            ...
            auto next_month = now + std::chrono::hours(24 * 33); // 33天确保跨月

            std::time_t time = std::chrono::system_clock::to_time_t(next_month);
            std::tm tm = *std::localtime(&time);

            char month_str[7];
            std::strftime(month_str, sizeof(month_str), "%Y%m", &tm);
            std::string partition_name = table_name + "_" + month_str;
            ...
            std::tm next_first_day = first_day;
            next_first_day.tm_mon += 1;
            std::mktime(&next_first_day);

            char from_date[11], to_date[11];
            std::strftime(from_date, sizeof(from_date), "%Y-%m-%d", &first_day);
            std::strftime(to_date, sizeof(to_date), "%Y-%m-%d", &next_first_day);

            // 3. 检查分区是否已存在
            int exists = 0;
            sess.get() << "SELECT 1 FROM pg_tables WHERE tablename = :name",
                    soci::into(exists), soci::use(partition_name);
            ...
            if (!exists) {
                ...
                comment << comment_prefix << " "
                        << std::string(month_str).insert(4, "年")
                        << "月";

                sess.get() << "COMMENT ON TABLE " << partition_name
                           << " IS '" << comment.str() << "'";

                Logger::Global().Info("已创建分区: " + partition_name +
                                      " (" + from_date + " ~ " + to_date + ")");
                tr.commit();
                return true;
            }

            Logger::Global().Debug("分区已存在: " + partition_name);
            return false;
        } catch (const std::exception& e) {
            Logger::Global().Error("创建分区失败: " + std::string(e.what()));
            return false;
        }
    }
    ...
    /**
     * 检查并创建未来N个月的分区（防御性设计）
     * @param months_ahead 预创建月数（默认3个月）
     */
    static void ensure_future_partitions(
            const std::string& table_name,
            int months_ahead = 3,
            const std::string& partition_key = "created_at",
            const std::string& comment_prefix = "日志分区"
        ...
        try {
            ...
            for (int i = 1; i <= months_ahead; ++i) {
                auto future = std::chrono::system_clock::now() +
                              std::chrono::hours(24 * 30 * i);

                std::time_t time = std::chrono::system_clock::to_time_t(future);
                std::tm tm = *std::localtime(&time);
                ...
                if (!exists) {
                    ...
                    Logger::Global().Info("已预创建分区: " + partition_name);
                }
            }
            tr.commit();
        } catch (const std::exception& e) {
            Logger::Global().Error("预创建分区失败: " + std::string(e.what()));
        }
    }

    static std::vector<std::string> check_partition_health(
            const std::string& table_name,
            const std::string& partition_key = "created_at"
    ) {
        std::vector<std::string> missing_months;
        try {
            Session sess;
            // 1. 获取当前时间范围
            std::tm now_tm;
            time_t now = std::time(nullptr);
            time_utils::safe_localtime(&now, &now_tm);
            // 2. 检查过去3个月和未来3个月
            for (int month_offset = -3; month_offset <= 3; ++month_offset) {
                std::tm tm = now_tm;
                tm.tm_mon += month_offset;
                std::mktime(&tm);

                char month_str[7];
                std::strftime(month_str, sizeof(month_str), "%Y%m", &tm);
                std::string expected_partition = table_name + "_" + month_str;

                // 检查分区是否存在
                int exists = 0;
                sess.get() << "SELECT 1 FROM pg_tables WHERE tablename = :name",
                        soci::into(exists), soci::use(expected_partition);

                if (!exists) {
                    missing_months.push_back(month_str);
                }
            }

            return missing_months;
        } catch (const std::exception& e) {
            Logger::Global().Error("分区健康检查失败: " + std::string(e.what()));
            return {};
        }
    }
     ...
     * 修复缺失分区
     */
    static void repair_missing_partitions(
            const std::string& table_name,
            const std::vector<std::string>& missing_months,
            const std::string& partition_key = "created_at"
    ) {
        try {
            Session sess;
            ...
            for (const auto& month : missing_months) {
                ...
                std::string partition_name = table_name + "_" + month;
                ...
                Logger::Global().Warning("已修复缺失分区: " + partition_name);
            }

            tr.commit();
        } catch (const std::exception& e) {
            Logger::Global().Error("修复分区失败: " + std::string(e.what()));
        }
    }
...
//        auto p = std::make_shared<soci::connection_pool>(5);
