...
SOCI_MAP(OrderDetail,"order_details",
         soci_helpers::make_pair(&OrderDetail::orderId, "order_id",true),
...
private:
    static void insertDemo() {
        ...
        if (auto flag = SociDB::insert(newUser)) {
            std::cout << "插入成功，ID: " << flag << std::endl;
        }
...
// 更新产品信息
    ...
    static void deleteDemo() {
        ...
        // 批量删除低价产品
        std::vector<int> productIdsToDelete;
      ...
      // 收集要删除的产品ID(保持不变)
       ...
       // 批量删除 - 修改为通过Product模板获取表信息
        ...
        std::vector<int> order_ids = {101, 102, 103, 104, 105};
...
// 辅助函数：查找原始价格
    static std::optional<double> findOriginalPrice(
            const std::vector<Product>& products, int id) {
