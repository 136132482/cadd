...
struct type_conversion {
    ...
    static size_t find_column_index(const soci::row& row, const std::string& col_name) {
        for(size_t i = 0; i < row.size(); ++i) {
            if (row.get_properties(i).get_name() == col_name) {
                return i;
            }
        }
...
namespace soci_config {
    inline bool is_standard_field(const std::string& name) {
        static const std::unordered_set<std::string> std_fields = {
                "id", "created_at", "updated_at", "is_delete"
        };
        return std_fields.count(name);
    }
}
...
namespace soci_helpers {
    ...
    struct TypeConverter<std::optional<T>> {
        static std::string toSQL(const std::optional<T>& value) {
            return value.has_value() ? TypeConverter<T>::toSQL(*value) : "NULL";
        }
    };
    ...
    struct TypeConverter {
        ...
        static std::string toSQL(const T& value) {
             if constexpr (is_chrono_time_point_v<T>) {
                auto tm = to_tm(value);
                std::ostringstream oss;
